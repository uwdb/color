...
@testset "exact symmetrical graphs" begin
    ...
    @testset "1-edge graph" begin
        ...
        update_node_lablels!(g_property, 1, Array([1]))
        update_node_lablels!(g_property, 2, Array([1]))
        add_labeled_edge!(g_property, Tuple([1, 2]), 1)
        summary = generate_color_summary(g_property, 16)
        query_graph = DiGraph(2)
        # q_property = DataGraph(query_graph, q_edge_labels, q_vertex_labels)
        q_property = QueryGraph(query_graph)
        update_node_lablels!(q_property, 1, Array([1]))
        update_node_lablels!(q_property, 2, Array([1]))
    ...
    @testset "looped query, no data labels" begin
        ...
        update_node_lablels!(g_property, 1, [1])
        update_node_lablels!(g_property, 2, [1])
        update_node_lablels!(g_property, 3, [1])
        update_node_lablels!(g_property, 4, [1])
        ...
        update_node_lablels!(q_property, 1, [1])
        update_node_lablels!(q_property, 2, [1])
        update_node_lablels!(q_property, 3, [1])
