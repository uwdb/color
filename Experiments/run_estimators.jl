...
function run_estimation_experiments(experiment_params_list::Vector{ExperimentParams})
    for experiment_params in experiment_params_list
        ...
        summary_file_location = "Experiments/SerializedSummaries/" * params_to_summary_filename(experiment_params.summary_params) *".obj"
        ...
        experiment_results = [("UpperBound", "Estimate", "LowerBound", "TrueCard", "EstimationTime")]
        for i in 1:length(all_queries[dataset])
            query = all_queries[dataset][i].query
            exact_size = all_true_cardinalities[dataset][i].exact_size
